...
% Long snake_case identifiers and file paths appear inline throughout; without this
% they are unbreakable boxes and overrun the margin.
\usepackage[htt]{hyphenat}
% Let TeX relax interword spacing only on lines it cannot otherwise set.
...
\textbf{Artifact:} Full code, configs, version-by-version analyses, per-run SQLite databases, and raw run directories are released with this paper (the ablation run's database was lost; its generated run report is released verbatim instead --- note that the report generator emits a fixed set of sections, so that file contains tables for conditions the ablation did not run, including a tutoring column that is 0\% throughout and learner-profile rows reading \texttt{unknown}; its L6 appendix also carries boilerplate that describes the scoring path inconsistently, see \S9). A day-by-day research journal recording what each generation was for and what changed afterwards is published separately at \url{https://kotowari-modoki.github.io/experimental-commons/research/agent-based-theory-testing-2-sigma-problem/}. The author's own generation-by-generation review of this work --- what was verified against the databases and what is taken on trust --- is released under \texttt{results/reviews/}. The released analysis documents carry all reported numbers and run identifiers, and the headline figures of every production run have been re-verified against the released databases.\\[0.5em]
...
\begin{tabular}{@{}lp{2.0cm}p{2.3cm}p{2.3cm}p{1.9cm}@{}}
\toprule
...
\text{rule\_order\_retention}$ the stored rule list is shuffled (this reorders the
\emph{listed} rules only; as \S9 records, it does \textbf{not} degrade the solving
procedure, which the memory keeps in a separate \texttt{strategy} field the
constraint never touches); memory is then trimmed to the word budget by dropping
...
\begin{enumerate}
  ...
  \item[(ii)] A \textbf{homogenization effect}: in v6, relative to a heterogeneous
    ...
    per-learner spread; the $\pm$25-point figure itself rests on a single learner
    per type per condition---8 tasks each, so $\pm$25 points is a 2-item
    difference---and should be read as a hypothesis, not an estimate.)
  \item[(iii)] \textbf{Misconception correction is decoupled from score}: v6 tutoring
    ...
    classroom, and the decoupling reappeared in v7b (generic tutoring: 100\%
    correction, 47\% accuracy) and v8.  We previously explained this by saying that
    correcting a \emph{propositional} misconception (``Blue is always active'') does
    nothing for a \emph{procedural} deficit; our own later audit of the learner-type
    constraints rules that explanation out (\S9), since the \texttt{order\_confused}
    constraint never produced a procedural deficit to begin with.  The decoupling
    itself is a stable observation across three generations; \textbf{we do not have an
    identified mechanism for it}, and we no longer offer one.
\end{enumerate}
...
v4--v9c2, the run-id$\leftrightarrow$database mapping, the \texttt{sessions} vs.\
\texttt{unique\_discussions} count per condition (F4), and the single-instance/diversity
status (F5).  Its summary matrix shows \textbf{F4 present in every generation up to
and including v9c, and resolved (5 independent discussions per condition) in v9c2};
F5b is present in all of v4--v9c and partially remediated in v9c2.  The two structural
confounds were therefore \emph{baked in from the first experiment} and silently shaped
six generations of results before the audit caught them.  (v8 initially appeared to be
a partial exception; separating the shared prerequisite lecture from the manipulated
phase showed its whole-class condition was a full instance of F4, with one discussion
shared by four learners, while its one-to-one condition was fully replicated.
Counting \texttt{learning\_sessions} without distinguishing phases had compressed the
difference.)  This is the cautionary core of the paper: the
...
and v9c2, and tied for lowest in v9c) is the more defensible comparative claim: the single largest
...
\begin{enumerate}
  \item \textbf{The education effect is real.}  No-education baselines sat at 0\%
    throughout, so the pipeline does measure learning: agents that receive no
    education phase cannot answer at all, and every educated condition lands far
    above them.  Whatever else follows, the instrument detects the presence of
    teaching.

  \item \textbf{Class size does not predict outcome.}  The ordering is non-monotone
    in every generation that varied it --- v9b: lecture-only $=$ pair (60\%) $>$
    small 48\% $>$ large 47\% $>$ medium 45\%; v9c: pair 65\% $>$ small 57\% $>$
    large 52\% $>$ medium 40\%; v9c2: medium (8 learners) 86\% $>$ large (16) 81\%
    $>$ pair (2) 80\% $>$ small (4) 76\%.  Neither ``smaller is better'' nor its
    inverse survives.  The \texttt{medium} condition was the \emph{lowest} discussion
    condition in three consecutive generations and the \emph{highest} in the
    controlled one, which is itself evidence that the earlier pattern tracked
    confounds rather than class size.

  \item \textbf{Ownership does not predict outcome.}  Ownership --- the degree to
    which a learner is called on and must articulate its reasoning --- was designed
    to vary from 0.56 (large class, 19\% of learners speak) to 3.0 (pair, all speak),
    and the manipulation verifiably worked: speaking rates match the design in every
    generation.  Outcome does not follow it.  In v9c2 the highest-scoring condition
    is \texttt{medium} at ownership 1.5 and the second-highest is \texttt{large} at
    0.56, while both ownership-3.0 conditions (pair, small) score below them.  The
    learner that speaks most is not the learner that scores best.

  \item \textbf{Discussion adds no measurable value over individual reflection.}
    Once lecture-only learners were given a matched learning opportunity (A6's
    self-reflection phase), they scored 83\% --- above three of the four discussion
    conditions.  Only \texttt{medium} exceeds them, by 3 points, below the 5-point
    margin the harness requires to flag added value.  The v9c pattern in which
    \emph{every} discussion condition towered over a 28\% lecture baseline did not
    survive either readiness control or the removal of the learning-budget
    asymmetry.

    Findings 2--4 are the substance of the negative result: the three structural
    variables Bloom's account rests on --- group size, individual engagement, and
    social interaction itself --- each failed to predict learning in the one
    generation where prerequisites were controlled and discussions were genuinely
    replicated.  We state them separately because they were tested separately, and
    because a null reported only as a flag value is easy to lose.
  ...
  \item \textbf{Two robust micro-mechanisms.}  (i) \emph{Homogenization}: one-to-one
    ...
    accuracy.  Both held across multiple generations and are candidate hypotheses for
    human study, stated here as agent-environment findings only.  We state (ii) as an
    observation without a mechanism: the explanation we previously gave --- that
    correction leaves \emph{procedural} deficits untouched --- does not survive our
    audit of the learner-type constraints (\S9).
...
\begin{itemize}
  ...
  \item \textbf{Readiness items overlap the teaching material.}  Two of the four
    readiness checks---\texttt{rc\_recall\_01} (\texttt{[Yellow, Green]} $\to$ 7) and
    \texttt{rc\_edge\_case\_01} (\texttt{[Red, Blue]} $\to$ 0)---appear verbatim as
    worked examples, answers included, in the fixed lecture.  Those same two are the
    highest-passing items in both generations that used the gate (v9c: edge\_case
    100\%, recall 68\%, against procedure\_order 59\% and rule\_interaction 50\%;
    v9c2: 100\% and 91\% against 85\% and 85\%).  We do not claim the overlap
    \emph{drives} the pass rate---the signature does not reproduce in the evaluation
    tasks, where a likewise-overlapping item (\texttt{l2\_edge\_case\_01}) scored
    17.6\% in v9c---but it does weaken ``genuinely held the prerequisites'' (\S7)
    for edge\_case and recall specifically, since those items can be passed by
    reproducing an answer the lecture supplied.  A clean gate would draw its items
    from sequences absent from the teaching material.
  \item \textbf{The L6 semantic judge's verdict never reaches the score.}  A8 has two
    halves: exclude L6 from the main score, and route it to an LLM judge.  The first
    half works.  The second does not reach the reported numbers:
    \texttt{answer\_correct} is set only on the exact-match path
    (\texttt{lib/scorer.rb}); the LLM branch in \texttt{lib/phases/evaluator.rb}
    never sets it.  In the ablation's smoke database, L6 rows carry
    \texttt{auto\_scored = 0} and judge totals of 11--20 out of 20 while
    \texttt{answer\_correct} is null---so every downstream count reads them as wrong,
    and the L6 appendix still prints 0.  The fix is a few lines but requires a re-run
    to take effect; consequently \textbf{no generation reported here has a validated
    L6 measurement}, and the L6 figures in all run reports should be read as ``not
    measured,'' not as ``0\%.''
  \item \textbf{One of the four learner types was never instantiated.}
    \texttt{order\_confused} is defined by \texttt{rule\_order\_retention = 0.2},
    implemented as shuffling the stored \texttt{rules} array.  That array is a list of
    facts; the solving procedure lives in a separate \texttt{strategy} field, which the
    constraint does not touch.  Inspecting v7b --- a generation run entirely with this
    type --- all twelve learners, including the eight in conditions that received no
    procedure instruction, retained the correct ``check activation, then apply
    modifiers'' order in \texttt{strategy}.  The intended procedural deficit therefore
    never existed, which also explains why the run's procedure-order error detector
    reported 0\% in every condition.  Two consequences: any claim resting on a
    \emph{procedural} deficit is withdrawn (\S5.1, \S8), and v7b tested interventions
    against a deficit no learner had.  The v9c2 learner-type spread is unaffected ---
    its endpoints are \texttt{edge\_case\_dropper} (96\%) and
    \texttt{passive\_listener} (58\%), with \texttt{order\_confused} in between at
    77\% --- but the type's label overstates what was manipulated.  Operationalizing it
    properly means constraining the field where the construct actually lives.
  ...
  \item \textbf{Stochasticity is not seed-controlled.}  The learner-type constraints
    ...
    The scale of that variation is worth stating concretely, because it bounds what
    any cross-generation comparison can carry: on the \emph{same} fixed lecture and
    the \emph{same} readiness check, the \texttt{rule\_interaction} pass rate was
    50\% in v9c, 85\% in v9c2, and \textbf{12\%} in the ablation --- a range four to
    five binomial standard errors wide at $n$=34, which we cannot attribute to
    sampling alone and have not otherwise explained.  A related pattern: the two runs
    whose readiness gate failed (v9c, 37 points; the ablation, 25 points) both have
    the same two conditions at the ends of their spread --- the two-learner pair and
    the four-learner lecture arm --- whereas the controlled run does not (v9c2 runs
    from medium at 86\% to small at 76\%).  Whatever produced the large early
    spreads, it did not survive readiness control, but neither have we isolated it.
...
\label{tab:run-index}
\footnotesize
\begin{tabular}{@{}lp{2.9cm}p{3.4cm}lrp{3.6cm}@{}}
\toprule
...
Full 36-character run UUIDs (the table above abbreviates them):
...
Gen & run\_id \\
\midrule
v4 & \texttt{19b39e20-6901-4c90-8f3c-e24b352f23ae} \\
...
\begin{tabular}{@{}lp{4.1cm}p{3.1cm}p{5.1cm}@{}}
\toprule
...
F4  & Pseudoreplication (unique\_discussions=1) & v4--v9c & A3: population-multiplication, n\_disc=5 (v9c2) \\
...
reproduce the cross-version audit table. Note that
\texttt{check\_replication.py} hashes \texttt{learning\_sessions} without
distinguishing phases, so on designs with a shared prerequisite lecture the
per-condition count must be read per phase (\S6).

The interpretation flags quoted in \S7 are threshold tests computed in
\texttt{lib/report.rb}; their definitions matter for reading a \texttt{false}.
\texttt{class\_size\_effect\_supported} is true only if the discussion conditions are
\emph{exactly} monotone decreasing in size --- a single inversion makes it false, and
four conditions land in exact order by chance about 4\% of the time, so
\texttt{false} is the expected value under both a null and a noisy real effect.
\texttt{discussion\_added\_value} is true if \emph{any} discussion condition exceeds
the baseline by more than 5 points, so a single small condition can set it.
\texttt{ownership\_effect\_supported} compares concordant against discordant pairs
across the conditions.  None is a significance test.  The Zarn Token reference lecture,
...
\newblock \url{https://doi.org/10.18653/v1/D16-1264}
