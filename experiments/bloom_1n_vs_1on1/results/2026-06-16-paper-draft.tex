...
\textbf{Artifact:} Full code, configs, per-run databases, and version-by-version analyses are released with this paper.
...
pipeline surfaced five families of confound (F1--F5, two of which decompose into
...
you were not taught.''}  This split is the methodological heart of the paradigm: in
pilot generations (v1--v2) where rules appeared in the evaluation prompt, every
condition scored 100\% (a complete ceiling), masking all pedagogical effects.
Removing the rules dropped the no-education baseline to 0\% and opened measurable
variance.
...
We summarize seven production generations.  All production runs use
\texttt{claude-sonnet-4-6} for every agent role (one v9b smoke run used
\texttt{claude-haiku-4-5}).  The no-education baseline scored 0\% wherever it was
run, confirming that the memory-only design removes the prior-knowledge ceiling; we
omit it from the tables below except where noted.
...
v9b introduced the \textbf{population-multiplication} design (instantiating
2/4/8/16-learner classes) and an explicit ownership metric, and found neither a
...
v9c2 applies A0/A3/A4/A5/A6/A8 together and re-asks the v9c questions under control.
...
\begin{enumerate}
  ...
  \item \textbf{Cost asymmetry is large and usually ignored.}  Tutoring and large
    discussions cost 4--24$\times$ the tokens of lecture/self-reflection for
    equal-or-worse accuracy.  Any agent-based pedagogy claim should report
...
obtained before them.  We release all code, configurations, per-run databases, and
version-by-version analyses so that the confound checklist --- and the cautionary
tale --- can be reused.
