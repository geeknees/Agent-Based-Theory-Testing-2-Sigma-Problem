...
**Artifact:** Full code, configs, version-by-version analyses, per-run SQLite databases, and raw run directories are released with this paper (the ablation run's database was lost; its generated run report is released verbatim instead — note that the report generator emits a fixed set of sections, so that file contains tables for conditions the ablation did not run, including a tutoring column that is 0% throughout and learner-profile rows reading `unknown`; its L6 appendix also carries boilerplate that describes the scoring path inconsistently, see §9). A day-by-day research journal recording what each generation was for and what changed afterwards is published separately at https://kotowari-modoki.github.io/experimental-commons/research/agent-based-theory-testing-2-sigma-problem/. The author's own generation-by-generation review of this work — what was verified against the databases and what is taken on trust — is released under `results/reviews/`. The released analysis documents carry all reported numbers and run identifiers, and the headline figures of every production run have been re-verified against the released databases.
**Status:** Working paper — not peer-reviewed.
**License:** MIT License (see LICENSE in repository root).
**AI involvement statement:** This manuscript and the underlying research artifact — the experiment code, the design iterations across all generations, the analyses, and the paper text itself — were produced end-to-end by LLM agents (Anthropic Claude; the experiments and drafting principally used `claude-sonnet-4-6`), operating under the direction of the human author, who framed the research question, approved each generation's design, and reviewed the outputs. The human author takes full responsibility for the content. The same model family also serves as the experimental subjects; we discuss this reflexivity in §9.
...
Each evaluation task carries two fields: a `learner_prompt` with the rules **removed**, and `hidden_rules` used **only** by the auto-scorer. The learner must answer from memory formed during the education phase, with an explicit instruction *"Use only your memory of the rules. Do not assume rules you were not taught."* This split is the methodological heart of the paradigm. In the earliest pilot (v1), evaluation tasks never contained the rules text, but memory formation was unconstrained (no word budget, no rule-exposure limit): the teacher's lecture recited the domain rules near-verbatim, the transcript carried them into memory largely intact, and the solver then answered from a memory that was effectively a copy of the rulebook — every condition scored 100% (a complete ceiling) by a different route than we originally assumed. In v2 the rules were additionally embedded directly in the evaluation-task prompt text, which reproduced the same ceiling through a second, more direct channel. Both routes mask pedagogical effects. Removing the rules from the evaluation prompt (v3 onward) and separating `learner_prompt` from `hidden_rules` dropped the no-education baseline to (near) 0% and opened measurable variance.
...
(Three further types — help_seeker, answer_first, example_memorizer — were defined in v6 and used in early generations.) Constraints are applied mechanically: each edge-case memory item is dropped independently with probability `1 − retention`; with probability `1 − rule_order_retention` the stored rule list is shuffled (this reorders the *listed* rules only; as §9 records, it does **not** degrade the solving procedure, which the memory keeps in a separate `strategy` field the constraint never touches); memory is then trimmed to the word budget by dropping lowest-priority fields first; and a `passive_listener` with question probability 0 never speaks in discussion (its turn is skipped with no model call). These constraints make learner heterogeneity *real* at the representation level rather than role-played.
...
We summarize the production generations v4–v9c below; v3 was the pilot that established the memory-only protocol (§4.2), and the confound-controlled v9c2 is treated separately in §7. (The "seven generations" of the title counts design generations — v3, v4, v5, v6, v7, v8, and the v9 series — with v7a/v7b as two experiments within one generation and v9b/v9c/v9c2 as iterations of one.) All production runs use `claude-sonnet-4-6` for every agent role (one v9b smoke run used `claude-haiku-4-5`). The no-education baseline scored 0% in every production run (v4 onward), confirming that the memory-only design removes the prior-knowledge ceiling; we omit it from the tables below except where noted. (In one earlier v3 pilot smoke test, before the production protocol was frozen, a no-education learner scored 1/8 by correctly judging a since-retired L5 counterexample-format task; post hoc inspection showed that task's true/false judgment was answerable by pure logical inference on the claim's structure, without any domain-rule knowledge, so it was replaced with the debugging-format L5 used in all production runs (§4.5). This pilot-era artifact does not affect any production run reported below.)
...
Three findings recur and survive into the later generations. (i) **Bloom's tutoring advantage never appears**: classroom matches or beats one-to-one in every generation. (ii) A **homogenization effect**: in v6, relative to a heterogeneous classroom, one-to-one tutoring raised the weakest learner type (passive_listener +25 points) and lowered the strongest (order_confused −25 points), compressing variance — tutoring made the group more uniform without raising the mean. (The variance-compression direction is consistent with v5, where tutoring also showed the highest per-learner spread; the ±25-point figure itself rests on a single learner per type per condition — 8 tasks each, so ±25 points is a 2-item difference — and should be read as a hypothesis, not an estimate.) (iii) **Misconception correction is decoupled from score**: v6 tutoring corrected diagnosed misconceptions 75% of the time yet did not outscore the classroom, and the decoupling reappeared in v7b (generic tutoring: 100% correction, 47% accuracy) and v8. We previously explained this by saying that correcting a *propositional* misconception ("Blue is always active") does nothing for a *procedural* deficit; our own later audit of the learner-type constraints rules that explanation out (§9), since the `order_confused` constraint never produced a procedural deficit to begin with. The decoupling itself is a stable observation across three generations; **we do not have an identified mechanism for it**, and we no longer offer one. The v6 "homogeneous classroom 94%" is the one apparent win for group instruction, but it is a confounded one: that condition's teacher was told the whole class shared a single misconception and to target it — an efficiency artifact of asymmetric design, not a clean format comparison.
...
The cross-version audit table (released as an artifact) records, for every generation v4–v9c2, the run-id↔database mapping, the `sessions` vs. `unique_discussions` count per condition (F4), and the single-instance/diversity status (F5). Its summary matrix shows **F4 present (✓) in every generation up to and including v9c, and resolved (5 independent discussions per condition) in v9c2**; F5b is present in all of v4–v9c and partially remediated in v9c2. The two structural confounds were therefore *baked in from the first experiment* and silently shaped six generations of results before the audit caught them. (v8 initially appeared to be a partial exception; separating the shared prerequisite lecture from the manipulated phase showed its whole-class condition was a full instance of F4, with one discussion shared by four learners, while its one-to-one condition was fully replicated. Counting `learning_sessions` without distinguishing phases had compressed the difference.) This is the cautionary core of the paper: the confounds were invisible at the level of "the code runs and produces plausible numbers," and only a replication-and-independence audit surfaced them.

---

## 7. v9c2: The Confound-Controlled Experiment

v9c2 applies A0/A3/A5/A6/A8 together (A4 — learner-profile diversity — was verified as already satisfied from v8 on) and re-asks the v9c questions under control. (run_id `dc1bdd27`; `claude-sonnet-4-6`; 154 learners; 2,376,593 tokens.)
...
1. **The education effect is real.** No-education baselines sat at 0% throughout, so the pipeline does measure learning: agents that receive no education phase cannot answer at all, and every educated condition lands far above them. Whatever else follows, the instrument detects the presence of teaching.

2. **Class size does not predict outcome.** The ordering is non-monotone in every generation that varied it — v9b: lecture-only = pair (60%) > small 48% > large 47% > medium 45%; v9c: pair 65% > small 57% > large 52% > medium 40%; v9c2: medium (8 learners) 86% > large (16) 81% > pair (2) 80% > small (4) 76%. Neither "smaller is better" nor its inverse survives. The `medium` condition was the *lowest* discussion condition in three consecutive generations and the *highest* in the controlled one, which is itself evidence that the earlier pattern tracked confounds rather than class size.

3. **Ownership does not predict outcome.** Ownership — the degree to which a learner is called on and must articulate its reasoning — was designed to vary from 0.56 (large class, 19% of learners speak) to 3.0 (pair, all speak), and the manipulation verifiably worked: speaking rates match the design in every generation. Outcome does not follow it. In v9c2 the highest-scoring condition is `medium` at ownership 1.5 and the second-highest is `large` at 0.56, while both ownership-3.0 conditions (pair, small) score below them. The learner that speaks most is not the learner that scores best.

4. **Discussion adds no measurable value over individual reflection.** Once lecture-only learners were given a matched learning opportunity (A6's self-reflection phase), they scored 83% — above three of the four discussion conditions. Only `medium` exceeds them, by 3 points, below the 5-point margin the harness requires to flag added value. The v9c pattern in which *every* discussion condition towered over a 28% lecture baseline did not survive either readiness control or the removal of the learning-budget asymmetry.

   Findings 2–4 are the substance of the negative result: the three structural variables Bloom's account rests on — group size, individual engagement, and social interaction itself — each failed to predict learning in the one generation where prerequisites were controlled and discussions were genuinely replicated. We state them separately because they were tested separately, and because a null reported only as a flag value is easy to lose.


5. **The headline numbers are best explained by confounds, not pedagogy — though the comparison is not a clean ablation.** The v9c→v9c2 contrast moved the condition spread from 37 to 10 points and flipped three interpretation flags. We are deliberately careful about the inference: v9c2 changed six remediations *at once* and raised n from 34 to 154, so we cannot attribute the collapse to any single control. The two mechanisms are also partly entangled by design — A3 (replication) inherently increases n, and a 37-point spread built on `unique_discussions = 1` per condition (F4) was, by definition, learner-response variance to a single discussion rather than a stable effect. So "we removed confounds and the effect collapsed" and "we added replication and N=1 noise averaged out" are, here, two descriptions of the *same* remediation rather than rival explanations. What we can claim firmly is the conditional: a spread that does not survive replication and prerequisite control was not safe to report as a pedagogical effect. What we cannot claim is a variable-isolated causal decomposition; a single-control ablation is the obvious next experiment (§9). We did run one such ablation — `bloom_v9c2_ablation_ndisc1` (run `a551c74d`; n_disc=1, all other v9c2 remediations ON) — and it produced a 25-point condition spread (pair 72% vs. lecture 47%), intermediate between v9c (37pp) and v9c2 (10pp), consistent with replication being a key factor. However, the ablation's readiness gate failed at 53%, so prerequisite knowledge was also not controlled; the two factors changed simultaneously and the result cannot cleanly isolate A3 alone.

6. **Learner heterogeneity is the dominant variable.** When prerequisites are controlled, *who the learner is* (its memory constraints) explains far more variance (38 points) than *how it was taught* (10 points). This is itself a substantive result: in this environment, instructional format is a second-order factor behind learner representation.

7. **Two robust micro-mechanisms.** (i) *Homogenization*: one-to-one tutoring compresses the outcome distribution (raising the weakest type, lowering the strongest) without raising the mean. (ii) *Correction-outcome decoupling*: high misconception-correction rates do not translate into higher accuracy. Both held across multiple generations and are candidate hypotheses for human study, stated here as agent-environment findings only. We state (ii) as an observation without a mechanism: the explanation we previously gave — that correction leaves *procedural* deficits untouched — does not survive our audit of the learner-type constraints (§9).

8. **Cost asymmetry is large and usually ignored.** In v9c2, discussion conditions spent 13–17× the education-phase tokens of lecture/self-reflection (35–46k vs. 2.7k) for equal-or-worse accuracy, and in earlier generations one-to-one tutoring cost ~4.3× the classroom lecture (v5: 35,095 vs. 8,180 tokens). Any agent-based pedagogy claim should report cost-normalized outcomes; otherwise the "expensive condition" can look better simply because it did more.
...
- **Readiness items overlap the teaching material.** Two of the four readiness checks — `rc_recall_01` (`[Yellow, Green]` → 7) and `rc_edge_case_01` (`[Red, Blue]` → 0) — appear verbatim as worked examples, answers included, in the fixed lecture. Those same two are the highest-passing items in both generations that used the gate (v9c: edge_case 100%, recall 68%, against procedure_order 59% and rule_interaction 50%; v9c2: 100% and 91% against 85% and 85%). We do not claim the overlap *drives* the pass rate — the signature does not reproduce in the evaluation tasks, where a likewise-overlapping item (`l2_edge_case_01`) scored 17.6% in v9c — but it does weaken "genuinely held the prerequisites" (§7) for edge_case and recall specifically, since those items can be passed by reproducing an answer the lecture supplied. A clean gate would draw its items from sequences absent from the teaching material.
- **The L6 semantic judge's verdict never reaches the score.** A8 has two halves: exclude L6 from the main score, and route it to an LLM judge. The first half works. The second does not reach the reported numbers: `answer_correct` is set only on the exact-match path (`lib/scorer.rb`); the LLM branch in `lib/phases/evaluator.rb` never sets it. In the ablation's smoke database, L6 rows carry `auto_scored = 0` and judge totals of 11–20 out of 20 while `answer_correct` is null — so every downstream count reads them as wrong, and the L6 appendix still prints 0. The fix is a few lines but requires a re-run to take effect; consequently **no generation reported here has a validated L6 measurement**, and the L6 figures in all run reports should be read as "not measured," not as "0%."
- **One of the four learner types was never instantiated.** `order_confused` is defined by `rule_order_retention = 0.2`, implemented as shuffling the stored `rules` array. That array is a list of facts; the solving procedure lives in a separate `strategy` field, which the constraint does not touch. Inspecting v7b — a generation run entirely with this type — all twelve learners, including the eight in conditions that received no procedure instruction, retained the correct "check activation, then apply modifiers" order in `strategy`. The intended procedural deficit therefore never existed, which also explains why the run's procedure-order error detector reported 0% in every condition. Two consequences: any claim resting on a *procedural* deficit is withdrawn (§5.1, §8), and v7b tested interventions against a deficit no learner had. The v9c2 learner-type spread is unaffected — its endpoints are `edge_case_dropper` (96%) and `passive_listener` (58%), with `order_confused` in between at 77% — but the type's label overstates what was manipulated. Operationalizing it properly means constraining the field where the construct actually lives.
- **F1 unresolved.** Corrective feedback remains static; adaptive per-error feedback is unimplemented.
- **Stochasticity is not seed-controlled.** The learner-type constraints (probabilistic edge-case dropping, rule-order shuffling) and LLM sampling run without fixed RNG seeds, so runs are not bit-reproducible. Analyses regenerate deterministically from each run's frozen outputs, but an exact re-run will differ; seeded constraint application is planned for the next generation. The scale of that variation is worth stating concretely, because it bounds what any cross-generation comparison can carry: on the *same* fixed lecture and the *same* readiness check, the `rule_interaction` pass rate was 50% in v9c, 85% in v9c2, and **12%** in the ablation — a range four to five binomial standard errors wide at n=34, which we cannot attribute to sampling alone and have not otherwise explained. A related pattern: the two runs whose readiness gate failed (v9c, 37 points; the ablation, 25 points) both have the same two conditions at the ends of their spread — the two-learner pair and the four-learner lecture arm — whereas the controlled run does not (v9c2 runs from medium at 86% to small at 76%). Whatever produced the large early spreads, it did not survive readiness control, but neither have we isolated it.
- **Reflexivity of AI authorship.** As disclosed in the front matter, this study was designed, executed, and written by LLM agents of the same model family as its experimental subjects, under human direction and review. We flag this as a potential source of shared blind spots — the auditor and the audited share a substrate — and note that the confound audit (§6) was itself agent-executed; independent human replication of the audit is the obvious external check.
...
| F3 | Discussion context not injected | v8, v9b, v9c | A0: inject learner memory (v9c2) |
| F4 | Pseudoreplication (unique_discussions = 1) | v4–v9c | A3: population-multiplication, n_disc = 5 (v9c2) |
...
Each generation has a version-pinned config (`experiments/bloom_1n_vs_1on1/config/config_v*.yml`) and run script (`scripts/run_experiment_v*.rb`). Reports regenerate from a frozen run database via `scripts/regen_report.rb`. Audit scripts `check_replication.py` (counts `unique_discussions` per condition) and `check_confounds.py` (single-instance and profile-diversity checks) reproduce the cross-version audit table. Note that `check_replication.py` hashes `learning_sessions` without distinguishing phases, so on designs with a shared prerequisite lecture the per-condition count must be read per phase (§6).

The interpretation flags quoted in §7 are threshold tests computed in `lib/report.rb`; their definitions matter for reading a `false`. `class_size_effect_supported` is true only if the discussion conditions are *exactly* monotone decreasing in size — a single inversion makes it false, and four conditions land in exact order by chance about 4% of the time, so `false` is the expected value under both a null and a noisy real effect. `discussion_added_value` is true if *any* discussion condition exceeds the baseline by more than 5 points, so a single small condition can set it. `ownership_effect_supported` compares concordant against discordant pairs across the conditions. None is a significance test. The Zarn Token reference lecture, evaluation tasks, and rubric are included under `domains/zarn_tokens/`.
