...
| 5 | **持ち越し A2(v6/v7b)** | **`order_confused` 型は一度も成立していなかった。** `rule_order_retention` は `rules` 配列を shuffle するだけで、手順が書かれた `strategy` フィールドは対象外。v7b の12名全員(手順教示を受けていない classroom/generic の8名を含む)の `strategy` に正しい順序が残っていた。<br>該当3箇所:<br>**l.118** 「with probability `1 − rule_order_retention` the ordered rule list is shuffled (**degrading procedural knowledge**)」→ **実装の説明として不正確**<br>**l.149** finding (iii) 「correcting a *propositional* misconception … does nothing for a *procedural* deficit (**shuffled rule order**)」<br>**l.232** §8-4(ii) 「correcting a stated misconception leaves **procedural deficits** … untouched」<br>→ **観測は生き残り、説明が崩れる**。誤解修正率と得点の乖離(v6 75%、v7b 100%)は DB 照合済みの事実であり finding (iii) 自体は保持できるが、**その理由を「手続き的欠損」に帰す説明は根拠を失う** | **fixed**(2026-09-22): 3箇所すべて修正。**l.118** は「reorders the *listed* rules only … does **not** degrade the solving procedure, which lives in a separate `strategy` field」に。**§5.1 (iii)** と **§8-7 (ii)** は観測を保持したうえで「**we do not have an identified mechanism for it, and we no longer offer one**」と明記。あわせて **§9 に「One of the four learner types was never instantiated」を新設**し、v7b 12名全員の `strategy` 保持・検出器 0% の説明・v9c2 の 38pp は端点が別 type のため影響を受けないことまで記載 |
...
| A5 | 帰属の議論 | **一部解決済み、一部は 2026-09-22 に追加処置**。「6つの是正を同時に適用したため単一の統制に帰属できない」「ablation の readiness も 53% で不合格」は §8・§9 が元から適切に扱っていた(所見10)。一方 **(a) readiness の run 間変動の大きさ**と **(b) spread の両端がどの条件か**は論文に記載が無く、(b) は**査読記録側の記述が誤っていた**(所見12・13) |
...
### 追加所見(2026-09-22、`fixed`/`deferred` の横断検証で発覚)

| # | 深刻度 | 内容 | 処置 |
|---|--------|------|------|
| 12 | **medium(査読記録自身の誤り)** | REVIEW-ablation のチェックリストに「**v9c(37pp)・ablation(25pp)・v9c2(10pp) のいずれも pair(n=2)と lecture(n=4)が両端**」と書いたが、**v9c2 について誤り**。DB 実測では v9c2 の両端は **medium 86.1% と small 75.6%** で、pair(80.0)と lecture(83.3)はその内側にある。<br>pair と lecture が両端に来るのは **v9c(65.0 / 27.5)と ablation(72 / 47)の2世代のみ**で、**どちらも readiness 不合格**である。統制された v9c2 ではこの構造が消える。<br>→ 訂正後の観察は論文の主張を**弱めるのではなく支持する**。査読記録の側に、DB で検算していない一般化を書いてしまった | **fixed**(2026-09-22): REVIEW-ablation のチェックリストと所見3を訂正 |
| 13 | medium | **readiness の run 間変動が論文に記載されていなかった。** 同一の固定講義・同一の readiness チェックを用いながら、`rule_interaction` の通過率は **v9c 50% → v9c2 85% → ablation 12%** と振れている。n=34 では二項標準誤差(p≈0.3 で約8pp)の4〜5倍の開きであり、「シードを固定していない」という §9 の記述だけでは規模が伝わらない。<br>この変動幅は**世代間比較の全体に関わる** | **fixed**(2026-09-22): §9 の「Stochasticity is not seed-controlled」に観測された変動幅と、spread の両端が readiness 不合格の2世代でのみ pair/lecture になる点を追記(md・tex 両方) |
...
**1・2 は masumi が通読後に自分の言葉で記入(2026-09-22)。3〜5 は査読記録からの整理。**

1. **この論文が主張していること(2つの貢献)**:
   Bloom の2シグマ問題を LLM を使って各種世代で検証した。**v4–v7 では Bloom の主張と逆転し、
   クラスルームのほうがスコアが高い**という結果だった。**さらに交絡を統制した v9c2 では
   条件間の差が 10pp に収縮し、クラスサイズ・主体性・討論の付加価値のいずれも効果を示さなかった
   (3つの null)**。
   一方でこれは一概に Bloom の主張を覆すものではなく、どちらかというと **LLM が人を再現する制約**を
   表すものである。そのうえで、**学習者の違い(異質性)のほうが学習結果に影響を与える**(38pp vs 10pp)、
   **トークンのコスト非対称性**(同じ結果でも消費が13〜17倍違う)という知見が出た。

2. **その根拠として何を示しているか**:
   LLM に知識を教えて、架空のゲーム(Zarn Tokens)の正答率で検証した。評価は **memory-only** で、
   教育フェーズで形成されたメモリだけを見て10問に答えさせる。加えて自分のパイプラインに対する
   交絡監査(F1–F5)を行い、それを是正した v9c2(154名・討論を条件あたり5回独立反復・
   readiness 90% 通過)で再検証した。

3. **査読を通じて自分が確認できたこと**:
   13項目すべてを DB・コードまで辿った。**論文本体の数値はすべて DB と一致**した
   (Appendix A の n 9件、§7 主結果表5条件、readiness 内訳4項目、learner type spread)。
   世代査読で見つかった数値誤り(v8 の講義長、v9c2 §2.1 のスモーク取り違え、v7b §3.3 の8セル、
   v9b の v8 比較)は**いずれも分析ドキュメント側で、論文には伝播していなかった**。
   §7・§8・§9 のヘッジは査読で独立に到達した留保と同じ内容を先に書いており、**過大主張はない**。
   AI involvement statement は自分の認識と一致することを確認した。

4. **自分が確認できていないこと・信頼で受け入れていること**:
   - **L6 はどの世代にも検証済みの測定値が存在しない**(A8 の semantic judge の判定が
     `answer_correct` に書かれない。§9 に明記済み)
   - **ablation は DB 紛失**により run report の集計を独立に再計算できない。
     本世代のみ垂直貫通が原理的に不可能
   - 各世代の methodology-addendum は読んだが**再検証はしていない**
   - トークン数はすべて chars/4 の推定値(±20%)。DB 未照合
   - DOI 2件の解決確認と `.tex` のコンパイルは未実施
   - readiness と評価タスクの重複について、**評価タスク側に同じ signature が出ない理由は不明**

5. **外部査読者に依頼する前にすべきこと**:
   - DOI 2件が解決することを確認する
   - `.tex` をコンパイルして体裁を確認する(ローカルに LaTeX 環境がないため別手段が要る)
   - `data/` にコミットされている **0バイトの空ファイル6件**を削除する
     (`experiment.db 19b39e20 v4` 等。DB の取り違えを疑わせる)
   - 次のリリース時に Zenodo の deposit description を更新する
   - `lib/report.rb` の修正(L6 Appendix の矛盾した説明文・存在しない条件の表)を行うか、
     注記のままとするかを決める
