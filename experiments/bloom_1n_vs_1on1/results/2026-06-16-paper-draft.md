...
**Artifact:** Full code, configs, version-by-version analyses, per-run SQLite databases, and raw run directories are released with this paper (the ablation run's database was lost; its generated run report is released verbatim instead). The released analysis documents carry all reported numbers and run identifiers, and the headline figures of every production run have been re-verified against the released databases.
