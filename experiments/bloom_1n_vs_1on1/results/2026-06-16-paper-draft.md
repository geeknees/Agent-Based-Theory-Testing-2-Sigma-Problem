...
- **Stochasticity is not seed-controlled.** The learner-type constraints (probabilistic edge-case dropping, rule-order shuffling) and LLM sampling run without fixed RNG seeds, so runs are not bit-reproducible. Analyses regenerate deterministically from each run's frozen outputs, but an exact re-run will differ; seeded constraint application is planned for the next generation.
